...
_CREDENTIAL_VALUE_PATTERNS = tuple(
    ...
    for pattern in (
        ...
        r"(?<![A-Za-z0-9_-])sk-(?:proj-)?[A-Za-z0-9_-]{16,}",
...
def contains_credential_like_value(value: str) -> bool:
    return any(pattern.search(value) for pattern in _CREDENTIAL_VALUE_PATTERNS) or any(
        _token_shaped(candidate) for candidate in _TOKEN_SHAPE_RE.findall(value)
    )
...
def redact_credential_like_text(value: str | None) -> str | None:
    ...
    redacted = _TOKEN_SHAPE_RE.sub(
        lambda match: "<redacted-credential>" if _token_shaped(match.group(0)) else match.group(0),
        redacted,
    )
    return redacted
...
class DesktopElement:
    ...
    def planner_payload(self) -> dict[str, Any]:
        ...
        return {
            ...
            "value": redact_credential_like_text(self.value),
