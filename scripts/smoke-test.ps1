...
& $pythonPath -m pytest -q --basetemp (Join-Path $projectRoot '.pytest-tmp')
if ($LASTEXITCODE -ne 0) { throw "Unit tests failed ($LASTEXITCODE)." }
& $pythonPath -m handsfree_pc --config $configPath simulate --independent --file (Join-Path $projectRoot 'examples\demo_commands.txt')
