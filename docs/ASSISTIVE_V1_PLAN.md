...
- 麦克风占用自动暂停（`app.auto_pause_when_microphone_busy`，默认开）：`handsfree_pc/mic_guard.py` 读 Windows 麦克风 ConsentStore 注册表（`LastUsedTimeStop == 0` 即正在采集），Zoom/Teams/腾讯会议/WeLink/微信通话/浏览器会议都在其中；每 3 秒轮询，发现别的程序在用就释放麦克风（模型不卸载）、停止转写和语音播报，显示"检测到 X 正在使用麦克风，已暂停监听"，对方释放后自动恢复。自身解释器和 `microphone_guard_ignore` 里的程序不算；启动超过 12 小时仍未写 Stop 的僵尸记录忽略，避免永久暂停。
- 开机自启：`scripts/install_autostart.ps1` 注册用户登录触发的计划任务 "HandsFreePC"（延迟 20 s，交互会话，失败自动重启 5 次，无时限），任务动作直接是 `.venv\Scripts\pythonw.exe -m handsfree_pc.cli --config config.local.yaml run`（无控制台窗口；`run` 发现没有控制台时把 stdout/stderr 追加到 `%LOCALAPPDATA%\HandsFreePC\logsun.log`）。让计划任务直接持有解释器进程，`Stop-ScheduledTask HandsFreePC` 才真的能停掉监听、任务状态才会显示 Running、失败重启才有意义——之前用 VBScript 启动器时启动器一退出任务就算完成，监听进程变成孤儿，停不掉也重启不了。`scripts/uninstall_autostart.ps1` 注销任务并顺手结束所有 `handsfree_pc.cli` 监听进程。`run` 有单实例锁（`logsun.lock`，按 pid 判活），第二个实例直接退出，避免两个监听者抢麦克风。
- 实测（2026-09-02，pythonw 计划任务）：`Start-ScheduledTask` 后约 14 s 出现 `MICROPHONE_READY`；用 Chrome `--use-fake-ui-for-media-stream` 打开一个 `getUserMedia` 页面模拟浏览器会议，启动 Chrome 后 3.4 s 记录 `MICROPHONE_GUARD_PAUSED`，页面释放麦克风后 1 s 内 `MICROPHONE_GUARD_RESUMED`；`Stop-ScheduledTask` 立即结束两个 python 进程，再 `Start-ScheduledTask` 正常恢复。注意 Windows 只把走 WASAPI 的采集记进 ConsentStore：Windows 自带的 pythonw/python 采集记在基础解释器 `C:\Python312\pythonw.exe` 名下（venv 的 exe 只是启动器），所以 guard 把 `sys._base_executable` 及其 python/pythonw 同目录文件都算作自己；而 winmm `waveInOpen` 这种老接口（例如从 pwsh 直接 P/Invoke）根本不会出现在注册表里，不能用来测试。
